...
| DBA - Maintenance - SQL Backup EW1P-OCT | Yes | 2026-07-06 | Succeeded | Sched1 | None | Backs up RDS instance EW1P-OCT directly to S3 bucket `ksys-ew1p-oct-dbbackup` path `backup/octopus_db_<date>.bak` via `rds_backup_database`. ⚠️ KMS key NULL — backup unencrypted at rest. |
| DBA - Maintenance - SQL Backups DIFF | Yes | 2026-07-06 | Succeeded | Daily @ 00:05 | dba@kurtosys.com | Ola Hallengren DIFF backup to D:\SQL\Backup\, then copies to S3 bucket `ksys-ew1r-db-backups` path `Backups/Reporting/EW1R-REP-01/` via USP_DatabaseBackupMoveToS3 (AWS CLI s3 sync via xp_cmdshell). ⚠️ No encryption specified in sync command. |
