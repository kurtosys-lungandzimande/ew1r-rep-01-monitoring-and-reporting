...
| Q4(C) — Who receives Slack alerts? | UPDATED | alerts-data-operations and alert-app-allow2fa-disabled are active Grafana alerts — not Zabbix webhooks. alerts-data-operations triggered by REP_CLIENT_CONFIG_CHANGES_REPORT (client config changes). alert-app-allow2fa-disabled triggered by REP_CLIENT_APP_AUTH_CONFIG_CHANGES_REPORT (2FA config changes). Both fed by DBA_VCC_MEMSQL_DAILY_CHECKS steps SP_AUDIT_FP_Client_Sizes_DETAILED and SP_AUDIT_FP_Client_ApplicationConfiguration_Auth_DETAILED. Alerts evaluate every 10 minutes. Data stale since May 2026 — alerts firing on stale data. Must be retired on decommission. |
...
| Q22 — Alerting solely dependent? | UPDATED | alerts-data-operations and alert-app-allow2fa-disabled are active Grafana alerts solely on this server — both will be silenced on decommission. SQL Server severity alerts all silent. Zabbix deadlock and sync check data also stops on decommission. |
