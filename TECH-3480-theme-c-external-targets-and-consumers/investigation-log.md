...
| Q4(C) — Who receives Slack alerts? | CLOSED | Confirmed via Grafana UI 2026-08-12. 2 contact points: alerts-data-operations (Slack, No attempts — never fired), grafana-default-email (Email, erroring — placeholder). alert-app-allow2fa-disabled does not exist as a contact point. No active consumer. Nothing to migrate on decommission. |
...
| Q22 — Alerting solely dependent? | CLOSED | Confirmed via Grafana UI 2026-08-12. alerts-data-operations has never fired (No attempts). alert-app-allow2fa-disabled does not exist as a contact point. Email receiver erroring with placeholder. No active alert consumer — no one loses visibility on decommission. |
