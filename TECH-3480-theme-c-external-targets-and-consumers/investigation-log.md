...
| Q4(C) — Who receives Slack alerts? | Slack contact points confirmed active in Grafana database — type: slack, encrypted token and webhook URL present for both alerts-data-operations and alert-app-allow2fa-disabled. Full alert pipeline documented. Slack channel names encrypted in database — confirm via Grafana UI (Alerting > Contact points). Both alerts must be retired on decommission. |
...
| Q22 — Alerting solely dependent? | Both Slack alerts confirmed active and will be silenced on decommission. Channel names to confirm via Grafana UI. SQL Server severity alerts all silent. Zabbix deadlock and sync check data also stops on decommission. |
